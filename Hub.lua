...
local MOBILE_MIN_W = 310
local MOBILE_MAX_W = 420
local MOBILE_MIN_H = 260
local MOBILE_MAX_H = 380
...
function Hub.novo(nome, tema, config)
	...
	gui.Name="HubUI_v72"; gui.ResetOnSpawn=false
	...
	local TOPBAR_H = IS_MOBILE and 42 or 48

	local SIDEBAR_W_FULL = IS_MOBILE and 88 or 162
	local SIDEBAR_W_MIN  = IS_MOBILE and 40 or 48

	local JANELA_W, JANELA_H
	if IS_MOBILE then
		JANELA_W = math.clamp(math.floor(vp.X * 0.94), MOBILE_MIN_W, MOBILE_MAX_W)
		JANELA_H = math.clamp(math.floor(vp.Y * 0.72), MOBILE_MIN_H, MOBILE_MAX_H)
	else
	...
	Cantos(janela, IS_MOBILE and 12 or 16)
	...
	Cantos(topbar, IS_MOBILE and 12 or 16); RegCor(topbar,"BackgroundColor3","Topbar")
	...
	local ponto = F({Size=UDim2.new(0,8,0,8),Position=UDim2.new(0,14,0.5,-4),
		BackgroundColor3=C.PontoA,ZIndex=12,Parent=topbar})
	Cantos(ponto,99); RegCor(ponto,"BackgroundColor3","PontoA")
	local pontoGlow = F({Size=UDim2.new(0,16,0,16),Position=UDim2.new(0,11,0.5,-8),
	...
	local BW = IS_MOBILE and 28 or 28
	local BH = IS_MOBILE and 28 or 28

	local lblNome = L({
		Size=UDim2.new(1,-(BW*2+60),1,0), Position=UDim2.new(0,30,0,0),
		Text=nome, TextColor3=C.Texto, Font=Enum.Font.GothamBold, TextSize=IS_MOBILE and 13 or 15,
		TextXAlignment=Enum.TextXAlignment.Left, ZIndex=12, Parent=topbar,
	})
	RegCor(lblNome,"TextColor3","Texto")

	local lblVer = L({
		Size=UDim2.new(0,50,0,12), Position=UDim2.new(0,30,0.5,4),
		Text="v7.2", TextColor3=C.Fraco, Font=Enum.Font.Gotham, TextSize=8,
		TextXAlignment=Enum.TextXAlignment.Left, ZIndex=12, Parent=topbar,
	})
	RegCor(lblVer,"TextColor3","Fraco")

	local btnMin = Instance.new("TextButton")
	btnMin.Size=UDim2.new(0,BW,0,BH); btnMin.Position=UDim2.new(1,-(BW*2+12),0.5,-BH/2)
	btnMin.Text="–"; btnMin.Font=Enum.Font.GothamBold; btnMin.TextSize=IS_MOBILE and 14 or 16
	...
	btnX.Size=UDim2.new(0,BW,0,BH); btnX.Position=UDim2.new(1,-(BW+6),0.5,-BH/2)
	btnX.Text="✕"; btnX.Font=Enum.Font.GothamBold; btnX.TextSize=IS_MOBILE and 10 or 12
	...
	if IS_MOBILE then
		...
		dragArea.Size=UDim2.new(1,-(BW*2+50),1,0); dragArea.Position=UDim2.new(0,28,0,0)
	...
	local NOTIF_W     = IS_MOBILE and math.min(JANELA_W - 16, 250) or 345
	local NOTIF_MAX   = 5
	local NOTIF_GAP   = 6
	local NOTIF_PAD_B = IS_MOBILE and 6 or 12
	...
	local notifHolder = F({
		...
		Position=UDim2.new(1,-(NOTIF_W+10),0,10),
	...
	function self:Notificar(titulo, mensagem, tipo, duracao, opcoes)
		...
		local icSize  = IS_MOBILE and 24 or 32
		local minH    = IS_MOBILE and 52 or 68
		local acaoH   = (acoes and #acoes>0) and (IS_MOBILE and 30 or 30) or 0
		...
		local halo2=F({Size=UDim2.new(0,icSize+20,0,icSize+20),Position=UDim2.new(0,10,0.5,-(icSize+20)/2),
			BackgroundColor3=cfg.cor,BackgroundTransparency=0.86,ZIndex=302,Parent=card}); Cantos(halo2,99)
		local halo1=F({Size=UDim2.new(0,icSize+10,0,icSize+10),Position=UDim2.new(0,15,0.5,-(icSize+10)/2),
			BackgroundColor3=cfg.cor,BackgroundTransparency=0.76,ZIndex=303,Parent=card}); Cantos(halo1,99)
		local ic=F({Size=UDim2.new(0,icSize,0,icSize),Position=UDim2.new(0,20,0.5,-icSize/2),
		...
		if icCustom then
			L({Size=UDim2.new(1,0,1,0),Text=icCustom,TextColor3=Color3.new(1,1,1),
				Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 11 or 15,ZIndex=308,Parent=ic})
		elseif tipo=="erro" then
			local sp=3; local cmp=IS_MOBILE and 11 or 15
			...
			L({Size=UDim2.new(1,0,1,0),Text=icCustom or cfg.icone,TextColor3=Color3.new(1,1,1),
				Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 12 or 16,ZIndex=308,Parent=ic})
		end

		local txtX    = 20+icSize+10
		local txtMaxW = -(txtX+14)

		local lblTitulo=L({Size=UDim2.new(1,txtMaxW,0,14),Position=UDim2.new(0,txtX,0,0),
			Text=titulo,TextColor3=Color3.new(1,1,1),Font=Enum.Font.GothamBold,
			TextSize=IS_MOBILE and 11 or 13,TextXAlignment=Enum.TextXAlignment.Left,
			TextWrapped=true,AutomaticSize=Enum.AutomaticSize.Y,ZIndex=306,Parent=card})
		local lblMsg=L({Size=UDim2.new(1,txtMaxW,0,12),Position=UDim2.new(0,txtX,0,0),
			Text=mensagem or "",TextColor3=cfg.corV,Font=Enum.Font.Gotham,
			TextSize=IS_MOBILE and 9 or 11,TextXAlignment=Enum.TextXAlignment.Left,
			TextWrapped=true,AutomaticSize=Enum.AutomaticSize.Y,ZIndex=306,Parent=card})
		...
		btnClose.Size=UDim2.new(0,18,0,18); btnClose.Position=UDim2.new(1,-22,0,5)
		btnClose.BackgroundTransparency=1; btnClose.Text="×"
		btnClose.Font=Enum.Font.GothamBold; btnClose.TextSize=13
		...
		if acoes and #acoes>0 then
			local totalBtns=math.min(#acoes,2); local gap2=5
			local btnW=(NOTIF_W-txtX-14-(totalBtns-1)*gap2)/totalBtns
			for i,acao in ipairs(acoes) do
				...
				ab.TextSize=IS_MOBILE and 9 or 11; ab.AutoButtonColor=false
		...
		task.defer(function()
			...
			local topPad=IS_MOBILE and 7 or 11; local gap=3; local botPad=IS_MOBILE and 9 or 14
			...
			ic.Position=UDim2.new(0,20,0,midY-icSize/2)
			halo1.Position=UDim2.new(0,15,0,midY-(icSize+10)/2)
			halo2.Position=UDim2.new(0,10,0,midY-(icSize+20)/2)
			...
			task.delay(0.2,function()
				...
				Tw(ic,0.22,{Size=UDim2.new(0,icSize+6,0,icSize+6)},Enum.EasingStyle.Back,Enum.EasingDirection.Out):Play()
	...
	Cantos(sidebar, IS_MOBILE and 10 or 12)
	...
	local topOffsetSidebar = IS_MOBILE and 6 or 40
	if not IS_MOBILE then
		local lblNav=L({
	...
	Pad(scrollTabs, IS_MOBILE and 4 or 6, IS_MOBILE and 6 or 10, IS_MOBILE and 3 or 6, IS_MOBILE and 3 or 6)
	local lyT=Instance.new("UIListLayout",scrollTabs)
	lyT.Padding=UDim.new(0, IS_MOBILE and 3 or 5)
	...
	pagArea=F({
		Size=UDim2.new(1,-(SW+4),1, IS_MOBILE and -6 or -10),
		Position=UDim2.new(0,SW+2,0, IS_MOBILE and 3 or 5),
	...
	if btnCollapse then
		...
		btnCollapse.MouseButton1Click:Connect(function()
			...
			Tw(pagArea,0.25,{
				Size=UDim2.new(1,-(targetW+4),1,-10),
	...
	function self:CriarAba(nomeAba, icone, badge)
		local BTN_H = IS_MOBILE and 34 or 42
		...
		Cantos(btn, IS_MOBILE and 7 or 9); RegCor(btn,"BackgroundColor3","Item")
		...
		Cantos(glow, IS_MOBILE and 7 or 9); RegCor(glow,"BackgroundColor3","Destaque")

		local ico=nil
		local lblX = IS_MOBILE and 6 or 10
		if icone then
			ico=L({Size=UDim2.new(0,18,1,0),Position=UDim2.new(0, IS_MOBILE and 5 or 8,0,0),
				Text=icone,TextColor3=C.Fraco,Font=Enum.Font.GothamBold,
				TextSize=IS_MOBILE and 12 or 16,ZIndex=3,Parent=btn})
			RegCor(ico,"TextColor3","Fraco"); lblX = IS_MOBILE and 24 or 34
		end

		local lbl=L({Size=UDim2.new(1,-(lblX+6),1,0),Position=UDim2.new(0,lblX,0,0),
			Text=nomeAba,TextColor3=C.Sub,Font=Enum.Font.Gotham,
			TextSize=IS_MOBILE and 10 or 12,TextXAlignment=Enum.TextXAlignment.Left,
			TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=3,Parent=btn})
		RegCor(lbl,"TextColor3","Sub")

		local badgeFr=nil; local badgeLbl2=nil
		if badge then
			badgeFr=F({Size=UDim2.new(0,16,0,12),Position=UDim2.new(1,-18,0.5,-6),
			...
			badgeLbl2=L({Size=UDim2.new(1,0,1,0),Text=tostring(badge),
				...
				TextSize=7,ZIndex=6,Parent=badgeFr})
		end
		...
		pagina.ScrollBarThickness=IS_MOBILE and 2 or 3
		...
		Pad(pagina, IS_MOBILE and 5 or 8, IS_MOBILE and 5 or 8, IS_MOBILE and 4 or 5, IS_MOBILE and 4 or 5)
		local lyPag=Instance.new("UIListLayout",pagina)
		lyPag.Padding=UDim.new(0, IS_MOBILE and 5 or 7)
		...
		local IH    = IS_MOBILE and 40 or 46
		local CBTN  = IS_MOBILE and 34 or 40
		local SLH   = IS_MOBILE and 58 or 68
		...
		function Aba:CriarSecao(titulo, iconeSecao)
			...
			local lb=L({
				...
				TextSize=IS_MOBILE and 8 or 9,Position=UDim2.new(0,10,0,0),ZIndex=2,Parent=fr,
			})
		...
		function Aba:CriarTexto(textoInicial, cor, tamanho)
			...
			local ts=tamanho or (IS_MOBILE and 11 or 13)
			local fr=F({Size=UDim2.new(1,-6,0,34),BackgroundColor3=C.Cartao,LayoutOrder=PO(),Parent=pagina})
			...
			lbl:GetPropertyChangedSignal("TextBounds"):Connect(function()
				local h2=math.max(34,math.ceil(lbl.TextBounds.Y/14)*16+12)
		...
		function Aba:CriarLink(texto, url, callback)
			...
			local _texto = texto
			local fr=F({Size=UDim2.new(1,-6,0,IS_MOBILE and 32 or 36),BackgroundColor3=C.Cartao,LayoutOrder=PO(),Parent=pagina})
			Cantos(fr,10); Stroke(fr,C.Borda,1,0.45); RC(fr,"BackgroundColor3","Cartao")
			local btn=Instance.new("TextButton")
			btn.Size=UDim2.new(1,0,1,0); btn.BackgroundTransparency=1
			btn.Text=""; btn.AutoButtonColor=false; btn.ZIndex=2; btn.Parent=fr
			L({Size=UDim2.new(0,18,1,0),Position=UDim2.new(0,8,0,0),
				Text="🔗",TextSize=IS_MOBILE and 10 or 12,Font=Enum.Font.Gotham,ZIndex=3,Parent=fr})
			local lbl=L({Size=UDim2.new(1,-32,1,0),Position=UDim2.new(0,26,0,0),
				Text=_texto,TextColor3=C.Destaque,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
		...
		function Aba:CriarCard(titulo, subtitulo, opcoes)
			...
			local CH=opcoes.altura or (IS_MOBILE and 56 or 64)
			...
			local lblIco=nil
			if iconeCard then
				lblIco=L({Size=UDim2.new(0,30,1,0),Position=UDim2.new(0,8+off,0,0),
					Text=iconeCard,TextColor3=C.Destaque,Font=Enum.Font.GothamBold,
					TextSize=IS_MOBILE and 15 or 20,ZIndex=2,Parent=wrap})
				RC(lblIco,"TextColor3","Destaque")
				off=off+32
			end

			local lblT=L({Size=UDim2.new(1,-(off+14),0,IS_MOBILE and 18 or 22),Position=UDim2.new(0,8+off,0,IS_MOBILE and 7 or 8),
			...
			local lblSub=nil
			if subtitulo then
				lblSub=L({Size=UDim2.new(1,-(off+14),0,IS_MOBILE and 14 or 18),Position=UDim2.new(0,8+off,0,IS_MOBILE and 25 or 28),
		...
		function Aba:CriarBadges(badges)
			local alturaTotal= IS_MOBILE and 28 or 32
			...
			for i,bd in ipairs(badges) do
				...
				local bfr=F({Size=UDim2.new(0,0,0,IS_MOBILE and 20 or 22),
					BackgroundColor3=Lerp3(bCor,Color3.new(0,0,0),0.65),
					LayoutOrder=i,Parent=wrap})
				Cantos(bfr,99); Stroke(bfr,bCor,1,0.35)
				local bl=L({Size=UDim2.new(1,-10,1,0),Position=UDim2.new(0,5,0,0),
					Text=txt,TextColor3=bCor,Font=Enum.Font.GothamBold,
					TextSize=IS_MOBILE and 8 or 10,ZIndex=2,Parent=bfr})
				task.defer(function()
					if bl.Parent then
						local tw2=bl.TextBounds.X+12
						bfr.Size=UDim2.new(0,tw2,0,IS_MOBILE and 20 or 22)
					end
				end)
		...
		function Aba:CriarProgresso(texto, valor, max, opcoes)
			...
			local PH= IS_MOBILE and 44 or 48
			...
			local lblT=L({Size=UDim2.new(0.6,0,0,16),Position=UDim2.new(0,12,0,IS_MOBILE and 7 or 9),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			RC(lblT,"TextColor3","Texto")
			local lblP=L({Size=UDim2.new(0.38,0,0,16),Position=UDim2.new(0.6,-4,0,IS_MOBILE and 7 or 9),
				Text=string.format("%d%%",math.floor(pct*100)),
				TextColor3=cor,Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Right,Parent=fr,
				Visible=mostrarPct})

			local trilha=F({Size=UDim2.new(1,-24,0,IS_MOBILE and 6 or 8),Position=UDim2.new(0,12,0,PH-(IS_MOBILE and 16 or 20)),
		...
		function Aba:CriarRating(texto, padrao, max, callback)
			...
			local RH= IS_MOBILE and 44 or 48
			local fr=F({Size=UDim2.new(1,-6,0,RH),BackgroundColor3=C.Cartao,
				LayoutOrder=PO(),Parent=pagina})
			Cantos(fr,10); Stroke(fr,C.Borda,1,0.35); RC(fr,"BackgroundColor3","Cartao")

			local lblTxt=L({Size=UDim2.new(0.5,0,0,16),Position=UDim2.new(0,12,0.5,-8),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			RC(lblTxt,"TextColor3","Texto")

			local STAR_SIZE= IS_MOBILE and 18 or 20; local STAR_GAP=3
			local totalW=(STAR_SIZE+STAR_GAP)*max-STAR_GAP

			local starsHolder=F({Size=UDim2.new(0,totalW,0,STAR_SIZE),
				Position=UDim2.new(1,-(totalW+12),0.5,-STAR_SIZE/2),
		...
		function Aba:CriarAvatar(userId, nomeExibido, subtituloExtra)
			...
			local AH= IS_MOBILE and 54 or 60
			...
			local AS= IS_MOBILE and 36 or 40
			local imgFr=F({Size=UDim2.new(0,AS,0,AS),Position=UDim2.new(0,8,0.5,-AS/2),
			...
			local function CarregarImagem(uid)
				...
				if uid then
					...
					iconLabel=L({Size=UDim2.new(1,0,1,0),Text="👤",TextSize=IS_MOBILE and 14 or 16,
			...
			local onl=F({Size=UDim2.new(0,9,0,9),Position=UDim2.new(1,-2,1,-2),
				BackgroundColor3=C.Sucesso,ZIndex=4,Parent=imgFr})
			Cantos(onl,99); Stroke(onl,C.Cartao,2,0)

			local txtOff=AS+16
			local lblN=L({Size=UDim2.new(1,-txtOff-12,0,IS_MOBILE and 15 or 16),
				Position=UDim2.new(0,txtOff,0,IS_MOBILE and 9 or 11),
				Text=_nome or "Usuário",TextColor3=C.Texto,
				Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,ZIndex=2,Parent=fr})
			RC(lblN,"TextColor3","Texto")

			local lblSubAv=nil
			if _sub then
				lblSubAv=L({Size=UDim2.new(1,-txtOff-12,0,13),Position=UDim2.new(0,txtOff,0,IS_MOBILE and 25 or 28),
					Text=_sub,TextColor3=C.Sub,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 9 or 11,
					TextXAlignment=Enum.TextXAlignment.Left,ZIndex=2,Parent=fr})
			...
			function obj:SetSubtitulo(v)
				...
				elseif v then
					lblSubAv=L({Size=UDim2.new(1,-txtOff-12,0,13),Position=UDim2.new(0,txtOff,0,IS_MOBILE and 25 or 28),
						Text=v,TextColor3=C.Sub,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 9 or 11,
						TextXAlignment=Enum.TextXAlignment.Left,ZIndex=2,Parent=fr})
				end
			end
			return obj
		...
		function Aba:CriarTabela(colunas, linhas)
			local ROW_H= IS_MOBILE and 26 or 28
			...
			local function Build(container, ls)
				...
				for i,col in ipairs(colunas) do
					L({Size=UDim2.new(colW,0,1,0),Position=UDim2.new(colW*(i-1),0,0,0),
						Text=col,TextColor3=C.Destaque,Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 8 or 10,
				...
				for ri,row in ipairs(ls) do
					...
					for ci,cell in ipairs(row) do
						...
						L({Size=UDim2.new(colW,0,1,0),Position=UDim2.new(colW*(ci-1),0,0,0),
							...
							TextSize=IS_MOBILE and 9 or 11,ZIndex=3,Parent=rFr})
					end
				end
			end
		...
		function Aba:CriarBotao(texto, callback, opcoes)
			...
			b.Text=""; b.Font=Enum.Font.GothamBold; b.TextSize=IS_MOBILE and 11 or 13
			...
			local lblHolder=F({Size=UDim2.new(1,-14,1,0),Position=UDim2.new(0,7,0,0),
				BackgroundTransparency=1,ZIndex=7,Parent=b})
			local lblTexto
			if icone then
				L({Size=UDim2.new(0,20,1,0),Position=UDim2.new(0,0,0,0),Text=icone,
					TextColor3=CorTexto(),Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 12 or 14,
					TextXAlignment=Enum.TextXAlignment.Left,ZIndex=7,Parent=lblHolder})
				lblTexto=L({Size=UDim2.new(1,-24,1,0),Position=UDim2.new(0,24,0,0),Text=texto,
					TextColor3=CorTexto(),Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 11 or 13,
					TextXAlignment=Enum.TextXAlignment.Left,ZIndex=7,Parent=lblHolder})
			else
				lblTexto=L({Size=UDim2.new(1,0,1,0),Text=texto,TextColor3=CorTexto(),
					Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 11 or 13,ZIndex=7,Parent=lblHolder})
			end
		...
		function Aba:CriarToggle(texto, padrao, callback, opcoes)
			...
			local TW2,TH2 = IS_MOBILE and 44 or 52, IS_MOBILE and 24 or 28
			local BR= TH2-6
			local fr=F({Size=UDim2.new(1,-6,0,IH),BackgroundColor3=C.Cartao,LayoutOrder=PO(),Parent=pagina})
			Cantos(fr,10); Stroke(fr,C.Borda,1,0.35); RC(fr,"BackgroundColor3","Cartao")

			local lblTxt=L({Size=UDim2.new(1,-(TW2+80),1,0),Position=UDim2.new(0,12,0,0),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			RC(lblTxt,"TextColor3","Texto")

			local trilha=F({Size=UDim2.new(0,TW2,0,TH2),Position=UDim2.new(1,-(TW2+10),0.5,-(TH2/2)),
			...
			local icoToggle=L({Size=UDim2.new(1,0,1,0),
				...
				TextSize=IS_MOBILE and 10 or 11,ZIndex=5,Parent=bolinha})

			local lblE=L({Size=UDim2.new(0,32,1,0),Position=UDim2.new(1,-(TW2+46),0,0),
				Text=estado and "ON" or "OFF",TextColor3=estado and C.Sucesso or C.Fraco,
				Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 9 or 11,Parent=fr})
		...
		function Aba:CriarSlider(texto, minV, maxV, padrao, callback, opcoes)
			...
			local labelY= IS_MOBILE and 7 or 9
			local lblTextoSlider=L({Size=UDim2.new(0.58,0,0,18),Position=UDim2.new(0,12,0,labelY),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			RC(lblTextoSlider,"TextColor3","Texto")
			local lblV=L({Size=UDim2.new(0.38,0,0,18),Position=UDim2.new(0.6,-4,0,labelY),
				Text=Formatar(val),TextColor3=corDestaque,Font=Enum.Font.GothamBold,
				TextSize=IS_MOBILE and 11 or 13,TextXAlignment=Enum.TextXAlignment.Right,Parent=fr})

			local hintY= IS_MOBILE and 25 or 28
			L({Size=UDim2.new(0,28,0,10),Position=UDim2.new(0,12,0,hintY),
				Text=Formatar(minV),TextColor3=C.Fraco,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 8 or 9,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			L({Size=UDim2.new(0,28,0,10),Position=UDim2.new(1,-40,0,hintY),
				Text=Formatar(maxV),TextColor3=C.Fraco,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 8 or 9,
				TextXAlignment=Enum.TextXAlignment.Right,Parent=fr})

			local trilhaY= IS_MOBILE and 40 or 47
			local trilhaH= IS_MOBILE and 5 or 6
			local trilha=F({Size=UDim2.new(1,-24,0,trilhaH),Position=UDim2.new(0,12,0,trilhaY),
				BackgroundColor3=C.Item,Parent=fr})
			RC(trilha,"BackgroundColor3","Item"); Cantos(trilha,99)
			...
			local BOLA= IS_MOBILE and 18 or 17
			...
			local tooltip=F({Size=UDim2.new(0,42,0,18),AnchorPoint=Vector2.new(0.5,1),
				Position=UDim2.new(pct,0,0,-5),BackgroundColor3=corDestaque,
			...
			local tooltipLbl=L({Size=UDim2.new(1,0,1,0),Text=Formatar(val),
				TextColor3=Color3.new(1,1,1),Font=Enum.Font.GothamBold,TextSize=9,ZIndex=7,Parent=tooltip})

			local hitH= IS_MOBILE and 34 or 28
			local hitbox=F({Size=UDim2.new(1,0,0,hitH),
				Position=UDim2.new(0,0,0.5,-hitH/2),
			...
			local function Upd(px)
				...
				tooltip.Position=UDim2.new(p2,0,0,-5)
		...
		function Aba:CriarDropdown(texto, opcoes, callback, cfg)
			...
			local maxVis=cfg.maxVisible or (IS_MOBILE and 4 or 5)
			local placeholderTxt=cfg.placeholder or opcoes[1] or "-"

			local aberto=false
			local IHd= IS_MOBILE and 32 or 34
			local FH= IS_MOBILE and 38 or 44
			local GAP=4; local PAD=6
			local SEARCH_H=useSearch and (IS_MOBILE and 30 or 32) or 0
			...
			L({Size=UDim2.new(0.45,0,1,0),Position=UDim2.new(0,12,0,0),Text=texto,
				TextColor3=C.Sub,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 10 or 12,
			...
			local lblS=L({Size=UDim2.new(0.42,0,1,0),Position=UDim2.new(0.5,-2,0,0),
				...
				TextSize=IS_MOBILE and 10 or 12,TextWrapped=true,
				TextXAlignment=Enum.TextXAlignment.Right,ZIndex=3,Parent=cab})
			RC(lblS,"TextColor3","Texto")

			local badge2=F({Size=UDim2.new(0,16,0,16),Position=UDim2.new(1,-40,0.5,-8),
			...
			local badgeLbl3=L({Size=UDim2.new(1,0,1,0),Text="0",TextColor3=Color3.new(1,1,1),
				Font=Enum.Font.GothamBold,TextSize=8,ZIndex=4,Parent=badge2})

			local seta=L({Size=UDim2.new(0,18,1,0),Position=UDim2.new(1,-22,0,0),
				Text="›",TextColor3=C.Fraco,Font=Enum.Font.GothamBold,
				TextSize=14,Rotation=90,ZIndex=3,Parent=cab})
			...
			local searchBox=nil
			if useSearch then
				local sfrRef=F({Size=UDim2.new(1,-14,0,SEARCH_H-5),Position=UDim2.new(0,7,0,FH+5),
					BackgroundColor3=C.Item,ZIndex=2,Parent=fr})
				Cantos(sfrRef,8); RC(sfrRef,"BackgroundColor3","Item")
				local sbrd=Stroke(sfrRef,C.Borda,1,0.4); RC(sbrd,"Color","Borda")
				L({Size=UDim2.new(0,16,1,0),Position=UDim2.new(0,5,0,0),
					Text="🔍",TextSize=IS_MOBILE and 10 or 11,Font=Enum.Font.Gotham,ZIndex=3,Parent=sfrRef})
				searchBox=Instance.new("TextBox")
				searchBox.Size=UDim2.new(1,-28,1,-6); searchBox.Position=UDim2.new(0,23,0,3)
				...
				searchBox.TextSize=IS_MOBILE and 10 or 11
			...
			listaHolder.ScrollBarThickness=IS_MOBILE and 2 or 3
			...
			local function ConstruirItens(filtro)
				...
				for _,op in ipairs(opcoes) do
					...
					local itLbl=L({Size=UDim2.new(1,-34,1,0),Position=UDim2.new(0,10,0,0),
						Text=op,TextColor3=ativo2 and C.BotaoTexto or C.Texto,
						Font=ativo2 and Enum.Font.GothamBold or Enum.Font.Gotham,
						TextSize=IS_MOBILE and 10 or 13,TextXAlignment=Enum.TextXAlignment.Left,
						ZIndex=4,Parent=it})
					local chk=L({Size=UDim2.new(0,18,1,0),Position=UDim2.new(1,-22,0,0),
						Text="✓",TextColor3=C.Sucesso,Font=Enum.Font.GothamBold,
						TextSize=IS_MOBILE and 11 or 14,ZIndex=4,Visible=ativo2,Parent=it})
				...
				if visiveis==0 then
					...
					L({Size=UDim2.new(1,0,1,0),Text="Nenhum resultado",TextColor3=C.Fraco,
						Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 10 or 12,ZIndex=3,Parent=empty})
				end
		...
		function Aba:CriarEntrada(placeholder, callback, opcoes)
			...
			local EH= IS_MOBILE and 40 or 44
			...
			local offX=10
			if icone then
				L({Size=UDim2.new(0,22,1,0),Position=UDim2.new(0,8,0,0),
					Text=icone,TextColor3=C.Fraco,Font=Enum.Font.GothamBold,
					TextSize=IS_MOBILE and 12 or 14,ZIndex=2,Parent=fr})
				offX=30
			end

			local tb=Instance.new("TextBox")
			tb.Size=UDim2.new(1,-(offX+10),1,-10); tb.Position=UDim2.new(0,offX,0,5)
			...
			tb.Font=Enum.Font.Gotham; tb.TextSize=IS_MOBILE and 11 or 13
			...
			if senha then
				local shown=false
				local dots=L({Size=UDim2.new(1,-(offX+36),1,0),Position=UDim2.new(0,offX,0,0),
					Text=string.rep("●",8),
					TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
					TextXAlignment=Enum.TextXAlignment.Left,ZIndex=3,Parent=fr})
				...
				eyeBtn.Size=UDim2.new(0,22,0,22); eyeBtn.Position=UDim2.new(1,-26,0.5,-11)
				eyeBtn.BackgroundTransparency=1; eyeBtn.Text="👁"
				eyeBtn.TextSize=IS_MOBILE and 12 or 14; eyeBtn.AutoButtonColor=false; eyeBtn.ZIndex=4; eyeBtn.Parent=fr
		...
		function Aba:CriarColorPicker(texto, padraoColor3, callback)
			...
			local FH2= IS_MOBILE and 38 or 42
			local CVW= IS_MOBILE and 130 or 190; local CVH= IS_MOBILE and 100 or 150
			local BAR_H= IS_MOBILE and 13 or 16; local PAD= IS_MOBILE and 8 or 10; local RPAD= IS_MOBILE and 8 or 10
			local RW= IS_MOBILE and 70 or 100; local RX=PAD+CVW+RPAD
			...
			L({Size=UDim2.new(0.5,0,1,0),Position=UDim2.new(0,12,0,0),Text=texto,
				TextColor3=C.Sub,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 10 or 12,
				TextXAlignment=Enum.TextXAlignment.Left,ZIndex=3,Parent=cab})

			local preview=F({Size=UDim2.new(0,IS_MOBILE and 20 or 24,0,IS_MOBILE and 20 or 24),Position=UDim2.new(1,-(IS_MOBILE and 46 or 54),0.5,-(IS_MOBILE and 10 or 12)),
				BackgroundColor3=corAtual,ZIndex=3,Parent=cab})
			Cantos(preview,6); Stroke(preview,C.Borda,1,0.4)

			local seta2=L({Size=UDim2.new(0,18,1,0),Position=UDim2.new(1,-22,0,0),
				Text="›",TextColor3=C.Fraco,Font=Enum.Font.GothamBold,
				TextSize=14,Rotation=90,ZIndex=3,Parent=cab})
			...
			local cvCursor=F({Size=UDim2.new(0,12,0,12),AnchorPoint=Vector2.new(0.5,0.5),
			...
			local bigPrev=F({Size=UDim2.new(0,RW,0,IS_MOBILE and 34 or 44),Position=UDim2.new(0,RX,0,PAD),
			...
			local hexH= IS_MOBILE and 22 or 26
			local hexFr=F({Size=UDim2.new(0,RW,0,hexH),Position=UDim2.new(0,RX,0,PAD+(IS_MOBILE and 38 or 48)),
			...
			hexBox.Size=UDim2.new(1,-6,1,-4); hexBox.Position=UDim2.new(0,3,0,2)
			...
			hexBox.TextSize=IS_MOBILE and 9 or 11; hexBox.TextXAlignment=Enum.TextXAlignment.Center
			...
			local PCOLS= IS_MOBILE and 4 or 6
			local PCELL= IS_MOBILE and 16 or 22
			local PGAP= IS_MOBILE and 3 or 4
			...
			local hueHit=Instance.new("TextButton"); hueHit.Size=UDim2.new(1,0,1,6); hueHit.Position=UDim2.new(0,0,0,-3)
			...
			local valHit=Instance.new("TextButton"); valHit.Size=UDim2.new(1,0,1,6); valHit.Position=UDim2.new(0,0,0,-3)
		...
		function Aba:CriarTeclaDeAtalho(texto, teclaPadrao, callback)
			...
			local KH= IS_MOBILE and 44 or 46
			local fr=F({Size=UDim2.new(1,-6,0,KH),BackgroundColor3=C.Cartao,LayoutOrder=PO(),Parent=pagina})
			Cantos(fr,10); Stroke(fr,C.Borda,1,0.35); RC(fr,"BackgroundColor3","Cartao")

			local BKW= IS_MOBILE and 60 or 88
			local BKH= IS_MOBILE and 28 or 28
			local reserva= BKW + (IS_MOBILE and 14 or 16)

			local lblTextoKb=L({Size=UDim2.new(1,-reserva,1,0),Position=UDim2.new(0,12,0,0),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			RC(lblTextoKb,"TextColor3","Texto")
			...
			if not IS_MOBILE then
				...
				btnEm.Size=UDim2.new(0,BKW,0,BKH)
				btnEm.Position=UDim2.new(1,-(BKW+8),0.5,-BKH/2)
				btnEm.BackgroundColor3=C.BotaoFundo; btnEm.TextColor3=C.BotaoTexto
				btnEm.Text="▶ Ativar"; btnEm.Font=Enum.Font.GothamBold; btnEm.TextSize=IS_MOBILE and 10 or 12
				...
				btnEm.MouseButton1Down:Connect(function()
					Tw(btnEm,0.07,{Size=UDim2.new(0,BKW-4,0,BKH-3)}):Play()
				end)
				btnEm.MouseButton1Up:Connect(function()
					Tw(btnEm,0.12,{Size=UDim2.new(0,BKW,0,BKH)},Enum.EasingStyle.Back):Play()
				end)
				btnEm.MouseButton1Click:Connect(function()
					Tw(btnEm,0.06,{BackgroundColor3=C.Destaque}):Play()
					task.delay(0.15,function()
						Tw(btnEm,0.15,{BackgroundColor3=C.BotaoFundo}):Play()
					end)
			...
			local obj={}
			function obj:Definir(kc) teclaAtual=kc end
		...
		function Aba:CriarContador(texto, minV, maxV, padrao, callback, opcoes)
			...
			local CH2= IS_MOBILE and 58 or 66
			local fr=F({Size=UDim2.new(1,-6,0,CH2),BackgroundColor3=C.Cartao,LayoutOrder=PO(),Parent=pagina})
			Cantos(fr,10); Stroke(fr,C.Borda,1,0.35); RC(fr,"BackgroundColor3","Cartao")

			local BW2= IS_MOBILE and 28 or 28
			local CTRL_Y= IS_MOBILE and 6 or 10
			...
			local topSec=F({Size=UDim2.new(1,-16,0,IS_MOBILE and 20 or 24),Position=UDim2.new(0,8,0,CTRL_Y),BackgroundTransparency=1,Parent=fr})
			L({Size=UDim2.new(0.6,0,1,0),Position=UDim2.new(0,0,0,0),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=topSec})
			RC(L({Size=UDim2.new(0.6,0,1,0),Position=UDim2.new(0,0,0,0),
				Text=texto,TextColor3=C.Texto,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,Parent=topSec}),"TextColor3","Texto")

			local ctrlRow=F({Size=UDim2.new(1,-16,0,BW2),Position=UDim2.new(0,8,0,CTRL_Y+(IS_MOBILE and 22 or 28)),BackgroundTransparency=1,Parent=fr})
			local ly=Instance.new("UIListLayout",ctrlRow)
			...
			local function MakeCtrlBtn(txt)
				...
				b2.Text=txt; b2.Font=Enum.Font.GothamBold; b2.TextSize=IS_MOBILE and 14 or 16
				b2.AutoButtonColor=false; b2.LayoutOrder=1; b2.Parent=ctrlRow; Cantos(b2,8)
				RC(b2,"BackgroundColor3","Item"); RC(b2,"TextColor3","Destaque")
				Stroke(b2,C.Borda,1,0.4)
				return b2
			end

			local btnMenos=MakeCtrlBtn("−")
			btnMenos.LayoutOrder=1

			local lblVal2=Instance.new("TextButton")
			lblVal2.Size=UDim2.new(0,IS_MOBILE and 52 or 60,0,BW2)
			lblVal2.BackgroundColor3=C.Item; lblVal2.AutoButtonColor=false
			lblVal2.Text=FormatarVal(val2); lblVal2.TextColor3=C.Texto
			lblVal2.Font=Enum.Font.GothamBold; lblVal2.TextSize=IS_MOBILE and 12 or 14
			lblVal2.LayoutOrder=2; lblVal2.Parent=ctrlRow
			Cantos(lblVal2,8); RC(lblVal2,"BackgroundColor3","Item"); RC(lblVal2,"TextColor3","Texto")
			Stroke(lblVal2,C.Borda,1,0.5)

			local btnMais=MakeCtrlBtn("+")
			btnMais.LayoutOrder=3

			local btnOk=MakeCtrlBtn("✓")
			btnOk.LayoutOrder=4
			...
			btnMenos.MouseEnter:Connect(function() Tw(btnMenos,0.1,{BackgroundColor3=C.ItemHover}):Play() end)
			btnMenos.MouseLeave:Connect(function() Tw(btnMenos,0.1,{BackgroundColor3=C.Item}):Play() end)
			btnMais.MouseEnter:Connect(function() Tw(btnMais,0.1,{BackgroundColor3=C.ItemHover}):Play() end)
			btnMais.MouseLeave:Connect(function() Tw(btnMais,0.1,{BackgroundColor3=C.Item}):Play() end)
			fr.MouseEnter:Connect(function() Tw(fr,0.12,{BackgroundColor3=C.ItemHover}):Play() end)
			fr.MouseLeave:Connect(function() Tw(fr,0.12,{BackgroundColor3=C.Cartao}):Play() end)
		...
		function Aba:CriarRadio(titulo, opcoes2, padrao, callback)
			...
			local RH2= IS_MOBILE and 32 or 34
			local RS= IS_MOBILE and 15 or 16
			...
			if titulo and titulo~="" then
				local hdr=F({Size=UDim2.new(1,0,0,IS_MOBILE and 24 or 30),BackgroundTransparency=1,Parent=wrap})
				local lbHdr=L({Size=UDim2.new(1,-20,1,0),Position=UDim2.new(0,12,0,0),
					Text=titulo,TextColor3=C.Sub,Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 9 or 10,
					TextXAlignment=Enum.TextXAlignment.Left,Parent=hdr})
				RC(lbHdr,"TextColor3","Sub")
			end
			...
			for _,op2 in ipairs(opcoes2) do
				...
				local circle=F({Size=UDim2.new(0,RS,0,RS),Position=UDim2.new(0,12,0.5,-RS/2),
				...
				local dot2=F({Size=UDim2.new(0,RS-7,0,RS-7),AnchorPoint=Vector2.new(0.5,0.5),
				...
				local lblOp=L({Size=UDim2.new(1,-(RS+24),1,0),Position=UDim2.new(0,RS+18,0,0),
		...
		function Aba:CriarExpansivel(titulo2, conteudo)
			...
			local aberto2=false
			local AH2= IS_MOBILE and 38 or 40
			...
			local lblTitExp=L({Size=UDim2.new(1,-36,1,0),Position=UDim2.new(0,12,0,0),Text=_titulo,
				TextColor3=C.Texto,Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 11 or 13,
				TextXAlignment=Enum.TextXAlignment.Left,ZIndex=3,Parent=cab2})
			RC(lblTitExp,"TextColor3","Texto")
			local chevron=L({Size=UDim2.new(0,18,1,0),Position=UDim2.new(1,-24,0,0),
				Text="›",TextColor3=C.Fraco,Font=Enum.Font.GothamBold,
				TextSize=14,Rotation=90,ZIndex=3,Parent=cab2})
			RC(chevron,"TextColor3","Fraco")

			local corpo2=F({Size=UDim2.new(1,-24,0,0),Position=UDim2.new(0,12,0,AH2+4),
		...
		function Aba:CriarAjusteTamanho(textoW, textoH)
			textoW = textoW or "Largura"
			textoH = textoH or "Altura"

			local minW_conf = IS_MOBILE and MOBILE_MIN_W or 400
			local maxW_conf = IS_MOBILE and MOBILE_MAX_W or 1200
			local minH_conf = IS_MOBILE and MOBILE_MIN_H or 300
			local maxH_conf = IS_MOBILE and MOBILE_MAX_H or 700

			local CH3= IS_MOBILE and 110 or 120
			local fr=F({Size=UDim2.new(1,-6,0,CH3),BackgroundColor3=C.Cartao,LayoutOrder=PO(),Parent=pagina})
			Cantos(fr,10); Stroke(fr,C.Borda,1,0.35); RC(fr,"BackgroundColor3","Cartao")

			local lblTitulo3=L({Size=UDim2.new(1,-16,0,IS_MOBILE and 18 or 20),Position=UDim2.new(0,12,0,IS_MOBILE and 6 or 8),
				Text="📐 Tamanho da Janela",TextColor3=C.Destaque,Font=Enum.Font.GothamBold,
				TextSize=IS_MOBILE and 10 or 12,TextXAlignment=Enum.TextXAlignment.Left,Parent=fr})
			RC(lblTitulo3,"TextColor3","Destaque")

			local SLH2= IS_MOBILE and 34 or 36

			local trilhaWfr=F({Size=UDim2.new(1,-24,0,SLH2),Position=UDim2.new(0,12,0,IS_MOBILE and 26 or 30),
				BackgroundTransparency=1,Parent=fr})

			local function MakeSliderRow(parent, yPos, labelTxt, valInicial, minSl, maxSl, onChanged)
				local rowFr=F({Size=UDim2.new(1,0,0,SLH2),Position=UDim2.new(0,0,0,yPos),BackgroundTransparency=1,Parent=parent})
				local lbR=L({Size=UDim2.new(0.4,0,0,SLH2),Position=UDim2.new(0,0,0,0),
					Text=labelTxt,TextColor3=C.Sub,Font=Enum.Font.Gotham,TextSize=IS_MOBILE and 9 or 11,
					TextXAlignment=Enum.TextXAlignment.Left,Parent=rowFr})
				RC(lbR,"TextColor3","Sub")
				local lbVal=L({Size=UDim2.new(0.2,0,0,SLH2),Position=UDim2.new(0.78,0,0,0),
					Text=tostring(valInicial),TextColor3=C.Texto,Font=Enum.Font.GothamBold,TextSize=IS_MOBILE and 9 or 11,
					TextXAlignment=Enum.TextXAlignment.Right,Parent=rowFr})
				RC(lbVal,"TextColor3","Texto")

				local barW= IS_MOBILE and 4 or 6
				local tril=F({Size=UDim2.new(0.36,0,0,barW),Position=UDim2.new(0.4,0,0.5,-barW/2),
					BackgroundColor3=C.Item,Parent=rowFr})
				RC(tril,"BackgroundColor3","Item"); Cantos(tril,99)
				local pct2=(valInicial-minSl)/(maxSl-minSl)
				local fillR=F({Size=UDim2.new(pct2,0,1,0),BackgroundColor3=C.Destaque,Parent=tril})
				Cantos(fillR,99); Grad(fillR,C.DestaqueV,C.Destaque,0)

				local BOLA2= IS_MOBILE and 16 or 14
				local bolaR=F({Size=UDim2.new(0,BOLA2,0,BOLA2),Position=UDim2.new(pct2,0,0.5,0),
					AnchorPoint=Vector2.new(0.5,0.5),BackgroundColor3=Color3.new(1,1,1),ZIndex=3,Parent=tril})
				Cantos(bolaR,99); Stroke(bolaR,C.Destaque,1.5,0.2)

				local hitR=F({Size=UDim2.new(1,0,0,IS_MOBILE and 32 or 24),
					Position=UDim2.new(0,0,0.5,-(IS_MOBILE and 16 or 12)),
					BackgroundTransparency=1,ZIndex=5,Parent=tril})

				local ativoR=false
				local currentVal=valInicial

				local function UpdR(px)
					local p=math.clamp((px-tril.AbsolutePosition.X)/tril.AbsoluteSize.X,0,1)
					local newVal=math.floor(minSl+(maxSl-minSl)*p+0.5)
					newVal=math.clamp(newVal,minSl,maxSl)
					currentVal=newVal
					local p2=(newVal-minSl)/(maxSl-minSl)
					fillR.Size=UDim2.new(p2,0,1,0)
					bolaR.Position=UDim2.new(p2,0,0.5,0)
					lbVal.Text=tostring(newVal)
					if onChanged then onChanged(newVal) end
				end

				hitR.InputBegan:Connect(function(i)
					if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
						ativoR=true; UpdR(i.Position.X)
						Tw(bolaR,0.1,{Size=UDim2.new(0,BOLA2+4,0,BOLA2+4)}):Play()
					end
				end)
				table.insert(hubSelf._conexoes, EntradaUsuario.InputChanged:Connect(function(i)
					if not ativoR then return end
					if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then
						UpdR(i.Position.X)
					end
				end))
				table.insert(hubSelf._conexoes, EntradaUsuario.InputEnded:Connect(function(i)
					if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
						if ativoR then
							ativoR=false
							Tw(bolaR,0.1,{Size=UDim2.new(0,BOLA2,0,BOLA2)}):Play()
						end
					end
				end))
				...
				return function(v)
					currentVal=v
					local p2=(v-minSl)/(maxSl-minSl)
					fillR.Size=UDim2.new(p2,0,1,0)
					bolaR.Position=UDim2.new(p2,0,0.5,0)
					lbVal.Text=tostring(v)
				end
			end

			local currW=JANELA_W
			local currH=JANELA_H

			local setW, setH

			setW = MakeSliderRow(trilhaWfr, 0, textoW, currW, minW_conf, maxW_conf, function(v)
				currW=v
				JANELA_W=v
				ALTURA_CHEIA=currH
				local curPosX=janela.Position.X.Offset
				local curPosY=janela.Position.Y.Offset
				local newPosX=math.clamp(curPosX, 0, vp.X-v)
				local newPosY=math.clamp(curPosY, 0, vp.Y-(minimizado and ALTURA_MIN or currH))
				Tw(janela,0.12,{
					Size=UDim2.new(0,v,0,minimizado and ALTURA_MIN or currH),
					Position=UDim2.new(0,newPosX,0,newPosY),
				},Enum.EasingStyle.Quart):Play()
				sidebar.Size=UDim2.new(0,math.min(SW, v*0.25), 1, 0)
				pagArea.Size=UDim2.new(1,-(math.min(SW,v*0.25)+4),1,-6)
			end)

			setH = MakeSliderRow(trilhaWfr, SLH2+6, textoH, currH, minH_conf, maxH_conf, function(v)
				currH=v
				ALTURA_CHEIA=v
				local curPosY=janela.Position.Y.Offset
				local newPosY=math.clamp(curPosY, 0, vp.Y-v)
				if not minimizado then
					Tw(janela,0.12,{
						Size=UDim2.new(0,currW,0,v),
						Position=UDim2.new(0,janela.Position.X.Offset,0,newPosY),
					},Enum.EasingStyle.Quart):Play()
				end
			end)

			local btnCenter=Instance.new("TextButton")
			btnCenter.Size=UDim2.new(1,-16,0,IS_MOBILE and 24 or 26)
			btnCenter.Position=UDim2.new(0,8,1,-(IS_MOBILE and 30 or 32))
			btnCenter.BackgroundColor3=C.Item; btnCenter.TextColor3=C.Destaque
			btnCenter.Text="⊙ Centralizar"; btnCenter.Font=Enum.Font.GothamBold
			btnCenter.TextSize=IS_MOBILE and 10 or 11; btnCenter.AutoButtonColor=false
			btnCenter.Parent=fr; Cantos(btnCenter,7)
			RC(btnCenter,"BackgroundColor3","Item"); RC(btnCenter,"TextColor3","Destaque")
			Stroke(btnCenter,C.Borda,1,0.4)

			btnCenter.MouseEnter:Connect(function() Tw(btnCenter,0.1,{BackgroundColor3=C.ItemHover}):Play() end)
			btnCenter.MouseLeave:Connect(function() Tw(btnCenter,0.1,{BackgroundColor3=C.Item}):Play() end)
			btnCenter.MouseButton1Click:Connect(function()
				local nx=math.floor((vp.X-currW)/2)
				local ny=math.floor((vp.Y-currH)/2)
				Tw(janela,0.35,{Position=UDim2.new(0,nx,0,ny)},Enum.EasingStyle.Back,Enum.EasingDirection.Out):Play()
				Tw(btnCenter,0.08,{Size=UDim2.new(1,-20,0,IS_MOBILE and 22 or 24)}):Play()
				task.delay(0.08,function()
					Tw(btnCenter,0.12,{Size=UDim2.new(1,-16,0,IS_MOBILE and 24 or 26)},Enum.EasingStyle.Back):Play()
				end)
			end)

			local obj={}
			function obj:ObterLargura() return currW end
			function obj:ObterAltura() return currH end
			function obj:DefinirLargura(v)
				currW=math.clamp(v,minW_conf,maxW_conf)
				setW(currW)
				JANELA_W=currW
				if not minimizado then
					Tw(janela,0.12,{Size=UDim2.new(0,currW,0,currH)},Enum.EasingStyle.Quart):Play()
				end
			end
			function obj:DefinirAltura(v)
				currH=math.clamp(v,minH_conf,maxH_conf)
				setH(currH)
				ALTURA_CHEIA=currH
				if not minimizado then
					Tw(janela,0.12,{Size=UDim2.new(0,currW,0,currH)},Enum.EasingStyle.Quart):Play()
	...
	function self:CriarDropdownTemas(aba)
		return aba:CriarDropdown(
			...
			{search=true, maxVisible=IS_MOBILE and 4 or 6, placeholder="Escolher tema..."}
		)
	end
